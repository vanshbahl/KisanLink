...
export function GlobalModeIndicator() {
  ...
  return (
    ...
    >
      ...
      <span>{isLive ? 'Live Backend Connected' : 'Demo / Prototype Mode'}</span>
      {showTooltip && (
        <div
          style={{
            position: 'absolute',
        ...
        >
          ...
          <p style={{ margin: '0.25rem 0 0 0', opacity: 0.85 }}>
            {isLive
              ? 'FastAPI PostgreSQL engine & OR-Tools logistics active. Local fallbacks used if endpoints time out.'
