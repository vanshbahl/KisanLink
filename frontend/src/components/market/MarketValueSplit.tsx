  ...
  return (
    <div className="mm-split">
      <div className="mm-split-row">
        <header><span>Through the mandi chain today</span><strong>{rupee(board.buyerCurrentPerKg)}<small>/kg</small></strong></header>
        <div className="mm-split-bar" role="img" aria-label={`Farmer ${rupee(board.mandiPricePerKg)} per kg, intermediaries ${rupee(spreadBefore)} per kg`}>
        ...
        <footer><span className="key key-farmer">Farmer</span><span className="key key-spread">Aggregator, mandi, wholesaler, retailer</span></footer>
      </div>

      <div className={`mm-split-row is-direct ${math.viable ? 'is-live' : ''}`}>
        <header><span>Direct through this market</span><strong>₹{hasMarket ? math.deliveredPerKg.toFixed(2) : '—'}<small>/kg</small></strong></header>
        <div className="mm-split-bar" role="img" aria-label={`Farmer ₹${math.farmerGatePerKg}, freight ₹${math.freightPerKg.toFixed(2)}, platform ₹${math.platformFeePerKg.toFixed(2)}`}>
        ...
        <footer>
          <span className="key key-farmer">Farmer</span>
          <span className="key key-freight">Transport</span>
          <span className="key key-platform">Platform {Math.round(board.platformFeePct * 1000) / 10}%</span>
          <span className="key key-saved">Buyer keeps</span>
        </footer>
      </div>
      ...
      <div className="mm-split-outcome">
        <article>
          ...
          <small>₹{math.farmerGatePerKg}/kg instead of ₹{board.mandiPricePerKg}/kg · +{math.farmerGainPct.toFixed(1)}%</small>
        </article>
        <article>
          <span>Buyers pay less</span>
          <strong>−<AnimatedNumber value={Math.max(0, math.buyerSavingTotal)} prefix="₹" /></strong>
          <small>₹{math.buyerSavingPerKg.toFixed(2)}/kg below the usual landed cost</small>
        </article>
        <article className="is-highlight">
          <span>Spread replaced by transparent cost</span>
          <strong><AnimatedNumber value={math.spreadRemovedPerKg} decimals={2} prefix="₹" suffix="/kg" /></strong>
          <small>₹{spreadBefore.toFixed(2)} margin becomes ₹{math.spreadAfterPerKg.toFixed(2)} of stated freight and fee</small>
        </article>
      </div>
    </div>
  )
