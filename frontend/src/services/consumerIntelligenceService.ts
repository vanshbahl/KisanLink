...
export function freshPick(listings: FarmerListing[]): ConsumerInsight {
  ...
  const ranked = active.map((item) => ({ item, score: (item.grade === 'Grade A+' ? 20 : 12) + Math.max(0, 18 - daysSince(item.harvestDate) * 8) + Math.min(18, item.remainingKg / 20) + (item.isUrgentRescue ? 6 : 0) + clamp((retailOf(item) - (item.rescueStatus === 'RESCUE_ACTIVE' ? item.rescueDiscountPricePerKg ?? item.pricePerKg : item.pricePerKg)) * 1.4, -12, 18) })).sort((a, b) => b.score - a.score)
  ...
  return { title: 'Fresh Pick', recommendation: `${selected.crop} from ${selected.farm} is the strongest current pick.`, confidence: clamp(68 + Math.round(ranked[0].score / 4), 68, 91), price, retail: retailOf(selected), listingId: selected.id, ctaLabel: 'View listing', note: selected.rescueStatus === 'RESCUE_ACTIVE' ? 'Rescue price shown; best suited for near-term use.' : undefined, factors: [{ label: 'Price', value: `₹${price}/kg · ₹${Math.max(0, retailOf(selected) - price)}/kg below local retail` }, { label: 'Harvest', value: daysSince(selected.harvestDate) === 0 ? 'Harvested today' : `Harvested ${daysSince(selected.harvestDate)} day${daysSince(selected.harvestDate) === 1 ? '' : 's'} ago` }, { label: 'Supply', value: `${selected.remainingKg} kg available` }, { label: 'Verification', value: 'Verified farmer' }] }
}

export function smartBuy(listing: FarmerListing): ConsumerInsight {
  const rescue = listing.isUrgentRescue || listing.rescueStatus === 'RESCUE_ACTIVE'; const price = rescue ? listing.rescueDiscountPricePerKg ?? listing.pricePerKg : listing.pricePerKg; const retail = retailOf(listing); const gap = retail - price; const age = daysSince(listing.harvestDate)
  const title = rescue ? 'Rescue Buy Check' : 'Smart Buy Analysis'; const judgement = rescue ? 'Best for near-term use' : gap >= 7 && age <= 1 ? 'Excellent buy' : gap >= 4 ? 'Good value' : gap >= 1 ? 'Fair price' : 'Premium price'
  return { title, recommendation: `${judgement} — ${listing.crop} is priced at ₹${price}/kg.`, confidence: clamp(72 + (listing.grade === 'Grade A+' ? 7 : 0) + (age <= 1 ? 5 : 0) + (listing.remainingKg > 20 ? 3 : 0), 60, 92), price, retail, ctaLabel: 'Add 3 kg to cart', note: rescue ? `₹${listing.pricePerKg}/kg normal price; discounted rescue price is active. Shorter selling window is indicated by the listing state.` : undefined, factors: [{ label: 'Local retail', value: `₹${retail}/kg — you save ₹${Math.max(0, gap)}/kg` }, { label: 'Harvest', value: age === 0 ? 'Harvested today' : `Harvested ${age} day${age === 1 ? '' : 's'} ago` }, { label: 'Available supply', value: `${listing.remainingKg} kg` }, { label: 'Grade & farm', value: `${listing.grade} · verified farmer` }] }
}
...
export function bestInResults(listings: FarmerListing[]): ConsumerInsight {
  ...
  const belowRetail = available.filter((item) => priceOf(item) < retailOf(item)).length
  return {
    ...
    factors: [
      { label: 'Price', value: `₹${price}/kg · ₹${Math.max(0, retailOf(chosen) - price)}/kg below local retail` },
      ...
      { label: 'Below local retail', value: `${belowRetail} of ${available.length} results` },
    ],
  }
}
