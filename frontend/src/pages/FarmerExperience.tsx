...
const crops = [
  { en: 'Fresh Tomatoes', hi: 'ताज़े टमाटर', image: '/assets/produce/tomato.webp', visual: 'tomato' as const, category: 'Vegetables' as const, mandi: 24, recommended: 32 , retail: 38 },
  { en: 'Baby Spinach', hi: 'बेबी पालक', image: '/assets/produce/spinach.webp', visual: 'leafy' as const, category: 'Vegetables' as const, mandi: 35, recommended: 42 , retail: 52 },
  { en: 'New Potatoes', hi: 'नए आलू', image: '/assets/produce/potato.webp', visual: 'potato' as const, category: 'Staples' as const, mandi: 21, recommended: 25 , retail: 32 },
  { en: 'Sharbati Wheat', hi: 'शरबती गेहूं', image: '/assets/produce/wheat.webp', visual: 'grain' as const, category: 'Grains' as const, mandi: 31, recommended: 37 , retail: 44 },
  { en: 'Sweet Carrots', hi: 'मीठी गाजर', image: '/assets/produce/carrot.webp', visual: 'root' as const, category: 'Vegetables' as const, mandi: 29, recommended: 36 , retail: 45 },
  { en: 'Red Onion', hi: 'लाल प्याज़', image: '/assets/produce/onion.webp', visual: 'onion' as const, category: 'Vegetables' as const, mandi: 18, recommended: 24 , retail: 30 },
]
const day = localDay
...
export function SellProducePage() {
  ...
  const chooseCrop = (crop: typeof crops[number]) => setForm((current) => ({ ...current, crop: crop.en, cropHi: crop.hi, imageSrc: crop.image, visual: crop.visual, category: crop.category, mandiPricePerKg: crop.mandi, retailPricePerKg: crop.retail, pricePerKg: crop.recommended }))
  ...
  const applyVoiceFields = (fields: {
    ...
    setForm((current) => ({
      ...
      retailPricePerKg: matched?.retail ?? current.retailPricePerKg,
...
function PriceStep({ form, update }: { form: FarmerListing; update: <K extends keyof FarmerListing>(key: K, value: FarmerListing[K]) => void }) {
  const { language, f } = useFeatureText(); const a = (key: Parameters<typeof aiText>[1], values?: Record<string, string | number>) => aiText(language, key, values)
  const earnings = form.quantityKg * form.pricePerKg; const extra = form.quantityKg * (form.pricePerKg - form.mandiPricePerKg)
...
export function FarmerOrdersPage() {
  const { language, f } = useFeatureText(); const a = (key: Parameters<typeof aiText>[1], values?: Record<string, string | number>) => aiText(language, key, values)
  const { showToast } = useToast(); const [orders, setOrders] = useState<FarmerOrder[] | null>(null); const [pickups, setPickups] = useState<Pickup[]>([]); const [filter, setFilter] = useState<OrderStatus | 'all'>('all')
  const load = () => { prototypeService.getOrders().then(setOrders); prototypeService.getPickups().then(setPickups) }; useEffect(() => { load() }, [])
...
function ForecastChart({ historical, forecast }: { historical: number[]; forecast: number[] }) {
  const all = [...historical, ...forecast]
  const min = Math.min(...all), max = Math.max(...all)
  const width = 600, height = 150, padY = 14
  const scaleY = (value: number) => height - padY - ((value - min) / Math.max(1, max - min)) * (height - padY * 2)
  const scaleX = (index: number) => (index / (all.length - 1)) * width
  const pointsFor = (values: number[], offset: number) => values.map((value, index) => [scaleX(index + offset), scaleY(value)] as const)
...
export function FarmerInsightsPage() {
  const { language, f } = useFeatureText()
  const a = (key: Parameters<typeof aiText>[1], values?: Record<string, string | number>) => aiText(language, key, values)
  const [crop, setCrop] = useState<FarmerCrop>('Tomatoes')
