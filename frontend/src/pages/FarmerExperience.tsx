...
export function FarmerProfilePage() {
  ...
  const switchDemo = async (role: 'consumer' | 'bulk') => { await switchRole(role); navigate(roleHome(role)) }
  return <div className="page farmer-feature-page">
    ...
    <section className="demo-switch-card farmer-demo-controls"><div><span className="eyebrow">{t('demoControls')}</span><h2>{t('switchDemoRole')}</h2><p>{t('switchDemoCopy')}</p></div><div className="demo-switch-options"><button onClick={() => switchDemo('consumer')}><span><ShoppingBasket size={22} /></span><strong>{t(roleKey.consumer)}</strong></button><button onClick={() => switchDemo('bulk')}><span><Building2 size={22} /></span><strong>{t(roleKey.bulk)}</strong></button><button onClick={() => { logout(); window.location.replace('/') }}><span><LogOut size={22} /></span><strong>{t('logOut')}</strong></button></div></section>
  </div>
}
