import { ArrowRight, BadgeIndianRupee, Leaf, ShieldCheck, Sprout, Truck } from 'lucide-react'
    ...
    <main className="welcome-page">
      ...
      <section className="welcome-hero">
        ...
        <div className="hero-visual" aria-label="Fresh produce from verified farms">
          <div className="hero-card hero-card-main"><ProduceArtwork emoji="🍅" visual="tomato" size="hero" /><div><span>Harvested today</span><h2>Fresh Tomatoes</h2><p>Ramesh Farms · Sonipat</p><strong>₹31 <small>/ kg</small></strong></div></div>
      ...
      <section id="demo" className="demo-entry">
        ...
        <div className="demo-role-grid">
          {([
            ['farmer', '🌾', 'Ramesh Kumar', 'Simple selling, orders & insights'],
            ['consumer', '🧺', 'Aarav Mehta', 'Fresh local produce & fair pricing'],
            ['bulk', '🏢', 'FreshKart', 'Nearby supply & procurement overview'],
          ] as Array<[Role, string, string, string]>).map(([role, emoji, name, description]) => (
            <button key={role} onClick={() => enterDemo(role)} disabled={Boolean(loadingRole)} className="demo-role-card">
              <span className={`demo-role-icon role-${role}`}>{emoji}</span>
