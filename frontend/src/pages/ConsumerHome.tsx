...
export function ConsumerHome() {
  ...
  return (
    <div className="page consumer-page">
      ...
      <MarketplaceAiSection sectionClassName="consumer-intelligence-slot" title="Fresh Pick" subtitle="One recommendation across every active farm listing near you." idleLabel="Find my pick" idleHint="Find the strongest current produce option" stages={['Checking fresh harvests', 'Comparing nearby farm prices', 'Comparing local retail prices', 'Reviewing available supply', 'Preparing your recommendation']} run={() => freshPick(intelligenceListings ?? [])} renderResult={(insight, reset) => <MarketplaceInsightResult {...insight} onClose={reset} onCta={() => insight.listingId ? navigate(`/consumer/listing/${insight.listingId}`) : reset()} footer="Uses the listing price, the local retail reference, harvest date, available stock and farm verification." />} />
