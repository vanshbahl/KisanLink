...
export function FarmerProducePage() {
  ...
  return (
    <div className="page">
      ...
      <div className="farmer-listing-grid">{data?.map((listing) => <article className="farmer-listing" key={listing.id}><ProduceArtwork imageSrc={listing.imageSrc} alt={listing.product} visual={listing.visual} size="mini" /><div><div><h2>{listing.product}</h2><StatusBadge tone="green">Active</StatusBadge></div><p><PackageOpen size={15} /> {listing.availableKg} kg available</p><strong>₹{listing.pricePerKg}/kg <small>Direct price</small></strong></div><Link to="/farmer/produce/details">Manage <ArrowRight size={16} /></Link></article>)}</div>
      <article className="gentle-banner"><Lightbulb size={25} /><div><strong>Buyer interest is highest for tomatoes this week.</strong><p>Keeping your quantity up to date helps us find the right buyers.</p></div><Link to="/farmer/insights">See insight</Link></article>
    </div>
  )
}
